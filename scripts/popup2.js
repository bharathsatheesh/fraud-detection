(function (ls) {
  ...
  function displaySimilarImages(url='https://static01.nyt.com/images/2019/01/13/opinion/13obs1web/13obs1web-square640.jpg?quality=75&auto=webp&disable=upscale&width=350') {
    $('#download_button').prop('disabled', true);

    var similar_images_table = $('#similar_images_table').empty();
    ...
    for (var rowIndex = 0; rowIndex < 10; rowIndex++) {
      ...
      for (var columnIndex = 0; columnIndex < columns; columnIndex++) {
        var index = rowIndex * columns + columnIndex;
        if (index === visibleImages.length) break;
        var image = '<td colspan="' + colspan + '" style="min-width: ' + ls.image_max_width + 'px; width: ' + columnWidth + '; vertical-align: top;"><img id="image' + index + '" src="' + url + '" /></td>';
        images_row.append(image);
      }
      similar_images_table.append(images_row);
    }
  }
